...
const SelectionView: React.FC<SelectionViewProps> = ({ onSelect, people }) => {
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-6 bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-indigo-900/20 via-slate-900 to-slate-900">
      <div className="max-w-5xl w-full text-center mb-16 animate-in fade-in slide-in-from-top-4 duration-700">
        ...
        <p className="text-xl text-slate-400 max-w-2xl mx-auto font-medium">
          Escolha uma área de atuação ou membro da equipe para iniciar.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full max-w-5xl animate-in fade-in slide-in-from-bottom-8 duration-700 delay-200">
        ...
        >
          ...
          <p className="text-indigo-100/70 mb-8 max-w-[200px] text-sm font-medium">Consolidado de todas as atividades e progresso do setor.</p>
        ...
        {people.map((person) => (
          ...
          >
            <div className={`p-4 rounded-2xl mb-8 ${person.name === 'Graziella' ? 'bg-indigo-600/10 text-indigo-500 border border-indigo-500/20' : 'bg-slate-700/50 text-slate-500 border border-slate-700'}`}>
              <User size={32} />
            </div>
