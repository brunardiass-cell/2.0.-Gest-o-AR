...
const ProjectsManager: React.FC<ProjectsManagerProps> = ({ projects, tasks, canEdit, onUpdate }) => {
  ...
  // ✅ helper: buscar projetos do Supabase e atualizar UI
  const reloadProjects = async () => {
    const { data, error } = await supabase.from('projects').select('*').order('name', { ascending: true });
    if (error) {
      console.error('Erro ao carregar projects do Supabase:', error);
      return;
    }
    ...
    if (!mapped.find(p => p.id === selectedProjectId)) {
      setSelectedProjectId(mapped[0]?.id || null);
    }
  };
  ...
  const addProject = async () => {
    ...
    const { error } = await supabase.from('projects').insert({
      ...
      tracking_checklist: newProj.trackingChecklist,
      regulatory_checklist: newProj.regulatoryChecklist,
    });

    if (error) {
      console.error('Erro ao criar project no Supabase:', error);
      return;
    }
    ...
    // ✅ recarrega do banco (mais confiável)
  ...
  const removeProject = async (id: string) => {
    ...
    if (confirm('Excluir este projeto e todos os seus checklists?')) {
      ...
      if (error) {
        console.error('Erro ao deletar project no Supabase:', error);
        return;
      }
      await reloadProjects();
    }
  ...
  const updateProjectStatus = async (status: ProjectStatus) => {
    ...
    const { error } = await supabase.from('projects').update({ status }).eq('id', selectedProjectId);
    if (error) {
  ...
  const addItemToChecklist = async () => {
    ...
    const isTracking = checklistType === 'tracking';
    const updatedTracking = isTracking ? [...current.trackingChecklist, newItem] : current.trackingChecklist;
    const updatedReg = !isTracking ? [...current.regulatoryChecklist, newItem] : current.regulatoryChecklist;

    const { error } = await supabase
      .from('projects')
      .update({
        tracking_checklist: updatedTracking,
        regulatory_checklist: updatedReg,
      })
      .eq('id', selectedProjectId);

    if (error) {
      console.error('Erro ao adicionar item no checklist (Supabase):', error);
      return;
    }
  ...
  const toggleItem = async (itemId: string, type: 'tracking' | 'regulatory') => {
    ...
    const updatedList = current[listKey].map(item =>
      item.id === itemId ? { ...item, completed: !item.completed } : item
    );

    const payload =
      type === 'tracking'
        ? { tracking_checklist: updatedList }
        : { regulatory_checklist: updatedList };

    const { error } = await supabase.from('projects').update(payload).eq('id', selectedProjectId);

    if (error) {
      console.error('Erro ao marcar item (Supabase):', error);
      return;
    }
  ...
  const removeItem = async (itemId: string, type: 'tracking' | 'regulatory') => {
    if (!canEdit || !selectedProjectId) return;
    ...
    const listKey = type === 'tracking' ? 'trackingChecklist' : 'regulatoryChecklist';
    const updatedList = current[listKey].filter(item => item.id !== itemId);
    ...
    if (error) {
      console.error('Erro ao remover item (Supabase):', error);
      return;
    }
        ...
        {canEdit && (
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            ...
            <div className="flex gap-2">
              ...
              />
              <button onClick={addProject} className="p-2 bg-indigo-600 text-white rounded-xl hover:bg-indigo-500 transition shadow-lg">
                <Plus size={20} />
              </button>
            </div>
          </div>
            ...
            {projects.map(p => (
              <button
                ...
                className={`w-full text-left p-4 transition-all flex items-center justify-between group ${
                  selectedProjectId === p.id ? 'bg-indigo-50 border-l-4 border-indigo-600' : 'hover:bg-slate-50 border-l-4 border-transparent'
                }`}
              >
                  ...
                  {canEdit && (
                    <div className="flex gap-2">
                      <input
                        ...
                        value={checklistType === 'tracking' ? newItemText : ''}
                      ...
                      />
                      <button onClick={() => { setChecklistType('tracking'); addItemToChecklist(); }} className="p-2 bg-slate-900 text-white rounded-xl">
                        <Plus size={16} />
                      </button>
                    </div>
                  )}
                  ...
                  {canEdit && (
                    <div className="flex gap-2">
                      <input
                        ...
                        value={checklistType === 'regulatory' ? newItemText : ''}
                        onChange={e => { setChecklistType('regulatory'); setNewItemText(e.target.value); }}
                        onKeyDown={e => e.key === 'Enter' && addItemToChecklist()}
                        className="flex-1 px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold"
                      />
                      <button onClick={() => { setChecklistType('regulatory'); addItemToChecklist(); }} className="p-2 bg-slate-900 text-white rounded-xl">
                        <Plus size={16} />
                      </button>
                    </div>
                  )}
            ...
            <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-8">
              ...
              <div className="space-y-3">
                {projectTasks.length > 0 ? projectTasks.filter(t => t.status !== 'Concluída').map(t => (
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">
                          {t.projectLead} • Prazo: {new Date(t.completionDate).toLocaleDateString('pt-BR')}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
                  ...
                  <p className="text-center py-8 text-slate-400 text-sm font-medium italic">Nenhuma atividade pendente para este projeto.</p>
                )}
              </div>
            </div>
          </>
